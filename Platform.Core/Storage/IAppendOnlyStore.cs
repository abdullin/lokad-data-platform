...
namespace Platform
{
    public sealed class FileAppendOnlyStore : IDisposable
    {
        ...
        public FileAppendOnlyStore(string directory)
        {
            ...
            _checkStream = new FileStream(Path.Combine(path, "stream.chk"), FileMode.OpenOrCreate, FileAccess.ReadWrite,
                FileShare.Read);
            ...
            _dataStream = new FileStream(Path.Combine(path, "stream.dat"), FileMode.OpenOrCreate, FileAccess.Write,
                FileShare.Read);
